...
type TComboboxDemoProps = {
  ...
  fonts?: TFontItem[];
  form: ReturnType<typeof useForm<z.infer<typeof dappSetting>>>;
};
              ...
              {fonts?.map((font) => (
                <CommandItem
                  ...
                  onSelect={(currentValue: string) => {
                    const isValidFont = fonts?.some(
                      (font) => font.family === currentValue,
                    );
                    if (isValidFont) {
                      setValue(currentValue === value ? "" : currentValue);
                      setOpen(false);
                      handleAddFont(currentValue);
                      form.setValue("fontOption", currentValue as EFontNames);
                    }
                  }}
                >
                  {font.family}
