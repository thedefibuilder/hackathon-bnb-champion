...
export const contractRouter = createTRPCRouter({
  ...
  getArtifacts: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const artifactKey = generateArtifactKey(input.moduleName, input.featureNames);
      ...
      const source = await getModuleSourceCode(input.moduleName, input.featureNames);
      const artifacts = await LlmService.buildCode(source);
      if (!artifacts.success) {
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: artifacts.message });
      }
